...
static void SetupCliArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-rpcport=<port>", strprintf("Connect to JSON-RPC on <port> (default: %u, testnet: %u, azcoin: %u, signet: %u, regtest: %u)", defaultBaseParams->RPCPort(), testnetBaseParams->RPCPort(), azcoinBaseParams->RPCPort(), signetBaseParams->RPCPort(), regtestBaseParams->RPCPort()), ArgsManager::ALLOW_ANY | ArgsManager::NETWORK_ONLY, OptionsCategory::OPTIONS);
...
private:
    ...
    std::string ChainToString() const
    {
        ...
        if (gArgs.GetChainName() == CBaseChainParams::AZCOIN) return " azcoin";
